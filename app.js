  ...
  if (heroSearchInput) {
    heroSearchInput.addEventListener('keydown', function(e) {
      if (e.key === 'Enter') { e.preventDefault(); performSearch(heroSearchInput.value); }
    });
  }

  /* ── Barra de búsqueda de la navegación ──────────────── */
  const navSearchInput = document.querySelector('.nav-search-input');
  if (navSearchInput) {
    navSearchInput.addEventListener('keydown', function(e) {
      if (e.key === 'Enter') { e.preventDefault(); performSearch(navSearchInput.value); }
    });
  }
